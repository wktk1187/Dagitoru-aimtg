...
export async function POST(request: NextRequest) {
  console.log(`[${new Date().toISOString()}] Request received at /api/slack/events entry point.`);
  const rawBody = await request.clone().text(); // 検証用にraw bodyを複製
  const reqForVerify = new NextRequest(request.url, {
    headers: request.headers,
    body: Buffer.from(rawBody),
    method: request.method,
    // @ts-ignore: Property 'duplex' does not exist on type 'RequestInit<NextRequestMeta>'.
    duplex: 'half' 
  });
